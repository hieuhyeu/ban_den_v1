...
app.post('/auth/register', async (req, reply) => {
  ...
  if (!supabase) return reply.code(503).send({ error: 'supabase_not_configured' })
...
function getBearer(req: { headers: Record<string, unknown> }) {
...
app.get('/me', async (req, reply) => {
  ...
  if (userRes.error) return reply.code(401).send({ error: userRes.error.message })

  let boardState: unknown = null
  const rpcRes = await supabase.rpc('get_board_state')
  if (!rpcRes.error) boardState = rpcRes.data
...
app.post('/players', async (req, reply) => {
  ...
  if (rpcRes.error) return reply.code(400).send({ error: rpcRes.error.message })
...
app.patch('/players/:id', async (req, reply) => {
  ...
  const playerId = z.string().uuid().safeParse((req.params as { id?: unknown }).id)
  if (!playerId.success) return reply.code(400).send({ error: 'invalid_player_id' })

  const patch: Record<string, unknown> = {}
  if (body.data.name !== undefined) patch.name = body.data.name
  if (body.data.colorKey !== undefined) patch.color_key = body.data.colorKey
  if (body.data.avatarUrl !== undefined) patch.avatar_url = body.data.avatarUrl
